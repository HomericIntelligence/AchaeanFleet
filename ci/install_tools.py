...
def install_archive(source: Path, expected: str, kind: str, member: str,
    ...
    if kind == "raw":
        ...
        with tarfile.open(source, "r:gz") as archive:
            ...
            with archive.extractfile(entry) as stream, destination.open("wb") as output:
...
def main() -> int:
    ...
    for name, variants in json.loads(args.artifacts.read_text()).items():
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            install_archive(archive, artifact["sha256"], artifact["format"],
                            artifact.get("member", name), args.destination / name)
